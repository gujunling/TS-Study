...
/**
 *
 * 元组：就是固定长度的数组,在定义的时候，类型和数据的个数在一开始就已经限定了,使用时的数据类型和数据个数应该和定义时的一致
 *
 * 语法： [类型1,类型2,类型3 ...]
 *
 */
...
j = ['qwe', 'aaa', 100.123456789]
console.log(j[0].split(''))
console.log(j[2].toFixed(2))

// ------------------------------- enum  枚举 ----------------------------------
// 因为数据库中转存字符串相对来说会大一点，为了使存储的数据相对来说小一点，会选择对某一些范围较小且不易改变的使用枚举
enum Gender {
  Male = 0,
  Female = 1
}
